...
void initLights() {
    lights[0] = Light(vec3(0.0, 5.0, 5.0), vec3(-1.0, -1.0, -1.0), vec3(1.0, 1.0, 1.0), .2, true);  // Directional light
    // Sun-like moving light source
    vec3 sunPos = vec3(50.0 * sin(iTime * sunSpeed), 10.0, 100.0 * cos(iTime * sunSpeed));
    lights[1] = Light(sunPos, vec3(0.0), vec3(1.0, 1.0, 1.0), 1.0, false); // Set as a moving point light
}
...
const float defaultAmbiantFactor = .5;
...
vec3 Shade(vec3 p, vec3 n, Ray eye, Material mat) {
    vec3 color = mat.ambient * mat.color; // Initial ambient component (no intensity here)
    vec3 specularTotal = vec3(0.0); // Store specular components
    vec3 diffuseTotal = vec3(0.0); // Store diffuse components
    ...
    for (int i = 0; i < numLights; i++) {
        ...
        if (light.isDirectional) {
            ...
            l = normalize(light.position - p);  // Point light
        }

        // Calculate shadow factor
        float shadow = Shadow(p, n, l);

        // Calculate diffuse component (apply light intensity here)
        vec3 diffuse = mat.diffuse * clamp(dot(n, l), 0.0, 1.0) * mat.color * light.color;

        // Calculate specular component (apply light intensity here)
        vec3 r = reflect(-l, n);
        vec3 specular = mat.specular * pow(clamp(dot(r, eye.d), 0.0, 1.0), 35.0) * light.color;

        // Add the contributions (diffuse and specular are affected by shadow)
        diffuseTotal += shadow * diffuse * light.intensity;  // Apply shadow and light intensity here
        specularTotal += shadow * specular * light.intensity;  // Apply shadow and light intensity here
    }

    // Combine all components: ambient, diffuse, and specular
...
float ModulateOcclusionByLight(vec3 p, vec3 n, vec3 l)
{
    ...
    // Modulate occlusion based on light intensity
    // If the point is fully lit, reduce the AO; if shadowed, keep full AO
    float filteredAO = mix(ao, 1.0, directLightIntensity);  // Reduce AO in lit areas
    return filteredAO;
}
...
{
    // First, compute the base shading with ambient occlusion applied
    vec3 color = Shade(p, n, eye, mat);  // Call ShadeWithAO1 once

    // Loop over all light sources and apply light-based occlusion modulation
    for (int i = 0; i < numLights; i++) {
        Light light = lights[i];

        // Compute the light direction for each light
        vec3 l = (light.isDirectional) ? normalize(-light.direction) : normalize(light.position - p);

        // Modulate the ambient occlusion based on the light intensity
        float ambientOcclusion = ModulateOcclusionByLight(p, n, l);

        // Apply light intensity and occlusion to diffuse and specular components
        color *= ambientOcclusion;// * Shade(p, n, eye, mat);
    }
...
void mainImage(out vec4 color,in vec2 pxy)
{
  ...
  if(hit)
  {
    ...
    rgb=ShadeWithAO2AndNestedReflection(p,n,ray, mat);
