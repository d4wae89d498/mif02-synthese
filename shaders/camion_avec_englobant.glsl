...
Val Box(vec3 p, vec3 c, vec3 r) {
    vec3 d = abs(p - c) - r;
    return Val(length(max(d, 0.0)) + min(max(d.x, max(d.y, d.z)), 0.0), 3);
}
...
Val Cylinder(vec3 p, vec3 c, float r, float h) {
    ...
    return Val(min(max(d.x, d.y), 0.0) + length(max(d, 0.0)), 7);
}
...
Val Torus(vec3 p, vec3 c, vec2 t) {
    vec3 q = p - c;
    return Val(length(vec2(length(q.xz) - t.x, q.y)) - t.y, 11);
}
...
Val object(vec3 p)
{
    ...
    // construction du camion camplet
...
void mainImage(out vec4 color,in vec2 pxy)
{
  ...
  float t=0.0;
  int s=0;
  int c=0; // This will store the cost
